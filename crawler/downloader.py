...
vl5x_cookie_script = r"""
            ...
            """
vl5x_js = execjs.compile(vl5x_cookie_script)


def getVl5xParam(cookie_vjkl5):
    return vl5x_js.call('getVl5x', cookie_vjkl5)
...
def generate_guid():
    return _create_guid() + _create_guid() + "-" + _create_guid() + "-" + _create_guid() + _create_guid() + "-" + _create_guid() + _create_guid() + _create_guid()
    # return '99005a2f-aab9-ed0511b0-ac5999c3f85e'
...
class Downloader():
    ...
    def download(self, request):
        ...
        print('start download request : ' + request.url)
        headers = {
        ...
        # url = 'http://wenshu.court.gov.cn/List/List?sorttype=1&conditions=searchWord+++2019-05-20%20TO%202019-05-21+%E4%B8%8A%E4%BC%A0%E6%97%A5%E6%9C%9F:2019-05-20%20TO%202019-05-21&conditions=searchWord+2+AJLX++%E6%A1%88%E4%BB%B6%E7%B1%BB%E5%9E%8B:%E6%B0%91%E4%BA%8B%E6%A1%88%E4%BB%B6'
        page_resp = requests.get(url=request.url, headers=headers)
        if(page_resp.status_code != 200):
            return None
        cookies = page_resp.cookies.get_dict()
        resp_text = page_resp.text
        scripts = resp_text[resp_text.index(
            '<script type="text/javascript">') + 31: resp_text.index('</script>')]
        # js中做的是动态计算的url，并使用window.location进行跳转，所以我只需要模拟window对象，
        # 并且实现里面所需要的方法， return出window.location对象 就可以拿到生成的url。就不需要破解里面的js了。
        scripts = r"""function atob(input) {
                        input = String(input);
                        input = input.replace(/[ \t\n\f\r]/g, '');
        ...
        js_function = execjs.compile(scripts)
        location = js_function.call('func')
        headers['Referer'] = request.url
        location = 'http://wenshu.court.gov.cn' + location
        page_resp = requests.get(
            location, headers=headers, cookies=cookies, allow_redirects=False)
        if(page_resp.status_code != 302):
            return Page.failed()
        cookies.update(page_resp.cookies.get_dict())

        # http://wenshu.court.gov.cn/List/List?sorttype=1&conditions=searchWord 1  民事案件 案件类型:民事案件
        if ('condition' in request.url):
            page_resp = requests.get(
                request.url, headers=headers, cookies=cookies, allow_redirects=False)
            if (page_resp.status_code != 200):
                return Page.failed()
            cookies.update(page_resp.cookies.get_dict())

            vl5x = getVl5xParam(cookies.get('vjkl5'))
            list_url = 'http://wenshu.court.gov.cn/List/ListContent'
            # 构造post请求参数
            # Param:搜索参数  Index:页数  Page:每页的数量  Order排序  Direction:升序降序  vl5x:加密参数(绑定cookie)  number:固定参数  guid:随机16进制字符串拼接
            list_body = {
                'Param': '案件类型:民事案件',
                'Index': 1,
                'Page': 20,
            ...
            list_resp = requests.post(
                list_url, data=list_body, headers=headers, cookies=cookies)
            # if (list_resp.status_code != 200):
            #     return None
            page = Page.mapping(list_resp)
            page.set_url(request.url)
            page.set_success(True)
            return page
        elif ('DocID' in request.url):
            doc_id = re.search('DocID=(.*)', request.url).group(1)
            contengt_url = 'http://wenshu.court.gov.cn/CreateContentJS/CreateContentJS.aspx?DocID=' + doc_id
            content_resp = requests.get(
                contengt_url, headers=headers, cookies=cookies)
            page = Page.mapping(content_resp)
            page.request.add_items(request.get_items())
            page.set_url(request.url)
            page.set_success(True)
            return page
        print('download url %s failed ' % request.url)
        return Page.failed()

# downloader = Downloader()
# downloader.download(Request('http://wenshu.court.gov.cn/List/List?sorttype=1&conditions=searchWord+1++%E6%B0%91%E4%BA%8B%E6%A1%88%E4%BB%B6+%E6%A1%88%E4%BB%B6%E7%B1%BB%E5%9E%8B:%E6%B0%91%E4%BA%8B%E6%A1%88%E4%BB%B6'))
